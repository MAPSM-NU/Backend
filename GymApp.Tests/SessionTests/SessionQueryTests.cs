...
namespace GymApp.Tests.SessionTests
{
    public class SessionQueryTests : TestBase
    {
        ...
        public async Task GetSessionMessagesDateTest()
        {
            ...
            var messages = await _sessionService.GetSessionMessages(new ClaimsPrincipal(), session.Id, DateTime.UtcNow.AddMinutes(-5).ToString(), DateTime.UtcNow.AddMinutes(11).ToString(), 1, "", "", "", 5);
        ...
        public async Task GetSessionMessagesDateInvalidTest()
        {
            ...
            var messages = await _sessionService.GetSessionMessages(new ClaimsPrincipal(), session.Id, "wrong date", "wrong date", 1, "", "", "", 5);
        ...
        public async Task GetSessionMessagesSearchTest()
        {
            ...
            _authorizationService.Setup(x => x.AuthorizeAsync(
                ...
                It.IsAny<string>()))
                .ReturnsAsync(AuthorizationResult.Success());
            var messages = await _sessionService.GetSessionMessages(new ClaimsPrincipal(), session.Id, "", "", 1, "", "", "Test", 5);
            Assert.Equal(5, messages.Data!.TotalCount);
            var messages2 = await _sessionService.GetSessionMessages(new ClaimsPrincipal(), session.Id, "", "", 1, "", "", "Test message 1", 5);
        ...
        public async Task GetSessionMessageSortTest()
        {
            ...
            _authorizationService.Setup(x => x.AuthorizeAsync(
                ...
                It.IsAny<string>()))
                .ReturnsAsync(AuthorizationResult.Success());
            var messagesAsc = await _sessionService.GetSessionMessages(new ClaimsPrincipal(), session.Id, "", "", 1, "date", "asc", "", 5);
            Assert.Equal("Test message 0", messagesAsc.Data!.Items.First().Content);
            var messagesDesc = await _sessionService.GetSessionMessages(new ClaimsPrincipal(), session.Id, "", "", 1, "date", "desc", "", 5);
